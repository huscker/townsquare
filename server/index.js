...
// USE_TLS explicitly enables/disables TLS. When unset, the default behaviour
// is preserved: TLS is used in every mode except "development".
const useTls =
  process.env.USE_TLS !== undefined
    ? process.env.USE_TLS === "true"
    : process.env.NODE_ENV !== "development";

const SERVER_PORT =
  parseInt(process.env.SERVER_PORT || process.env.PORT, 10) || 8080;
const CERT_FILE = process.env.CERT_FILE || "cert.pem";
const KEY_FILE = process.env.KEY_FILE || "key.pem";

// Allowed origins for WebSocket connections. Can be overridden with ALLOWED_ORIGINS
// env var (a regex string). Defaults to the original hardcoded pattern.
...
const server = useTls ? https.createServer(options) : http.createServer();
const wss = new WebSocket.Server({
  ...(process.env.NODE_ENV === "development" ? { port: 8081 } : { server }),
  verifyClient: (info) =>
    info.origin && !!info.origin.match(ALLOWED_ORIGINS_REGEX),
});
